...
Agent (Copilot / Claude / VS Code) ──HTTPS──► Quelch MCP
                                              │
                                              │ Authorization: Bearer <key>  (v1 — API key)
                                              │ Authorization: Bearer <jwt>  (v1.x — Entra)
                                              ▼
                                        accepts/rejects
```
...
| Agent → MCP | API key (v1) or Entra ID bearer token (v1.x) | [mcp-api.md "Authentication"](mcp-api.md#authentication). |
